...
# Addressing Critiques from AHBP Reviewer

## Testing for Differences Between Coupled and Individual Participants

It is possible that participants who's partners completed the survey differ systematically from those whose partners who did not complete the study. To test this, we will run a number of different t tests to see if there are significant differences in characteristics between the partners.

```{r}
# Testing for differences in characteristics between those whose partners completed and those whose partners did not complete
  # For age
t.test(data = individualsdata, age ~ partner_completed, var.equal = FALSE)
  # For SSS
t.test(data = individualsdata, SSS ~ partner_completed, var.equal = FALSE)
  # For SR_dom_cheng
t.test(data = individualsdata, SR_dom_cheng ~ partner_completed, var.equal = FALSE)
  # For SAT
t.test(data = individualsdata, SAT ~ partner_completed, var.equal = FALSE)
  # For IPIP_dom
t.test(data = individualsdata, IPIP_dom ~ partner_completed, var.equal = FALSE)
  # For facial_dominance
t.test(data = individualsdata, facial_dominance ~ partner_completed, var.equal = FALSE)
  # For facial_masculinityfemininity
t.test(data = individualsdata, facial_masculinityfemininity ~ partner_completed, var.equal = FALSE)
  # For facial_attractiveness
t.test(data = individualsdata, facial_attractiveness ~ partner_completed, var.equal = FALSE)
```
...
Now we will test the hypothesis with the standardized residuals for facial dominance that we created within sexes, while controlling `m_num_bio_child`. Because there are group differences in facial dominance between the neutral and non-neutral facial expression groups, we will conduct this analysis on both the full dyadic dataset and on the neutral faces only.

#### Full Dataset

Now we will fit our two models, one with the interaction term and one without (both with `m_num_bio_child` as a covariate).

```{r}
# Fitting the model with only the main effects of residual facial dominance for mothers and fathers
res_fac_dom_main_effects_model_n <- glm(shared_child_sex ~ z_m_res_facial_dominance + z_f_res_facial_dominance + m_num_bio_child, family = binomial(link = logit), data = dyadic_data)

# Fitting the model with the main effects of residual facial dominance for mothers and fathers as well as their interaction
res_fac_dom_interaction_model_n <- glm(shared_child_sex ~ z_m_res_facial_dominance + z_f_res_facial_dominance + m_num_bio_child + z_m_res_facial_dominance:z_f_res_facial_dominance, family = binomial(link = logit), data = dyadic_data)
```
...
```{r}
# Plotting Cook's distance for the main effects model
plot(res_fac_dom_main_effects_model_n, which = 4, id.n = 10)

#Plotting Cook's distance for the interaction model
plot(res_fac_dom_interaction_model_n, which = 4, id.n = 10)
```
...
```{r}
# VIF and tolerance for the main effects model
vif(res_fac_dom_main_effects_model_n)
1/vif(res_fac_dom_main_effects_model_n)
```
...
```{r}
# Producing the summary of the main effects model
summary(res_fac_dom_main_effects_model_n)

# Calculating the chi-squared statistic for the model compared to the baseline model with only the intercept
Chi_res_fac_dom_main_effects_model_n <- res_fac_dom_main_effects_model_n$null.deviance - res_fac_dom_main_effects_model_n$deviance
Chi_res_fac_dom_main_effects_model_n

# Calculating the degrees of freedom for the chi-square statistic comparing the model to it's baseline
df_res_fac_dom_main_effects_model_n <- res_fac_dom_main_effects_model_n$df.null - res_fac_dom_main_effects_model_n$df.residual
df_res_fac_dom_main_effects_model_n

# Conducting a chi-square test for the p-value using the chi-square statistic and the degrees of freedom
prob_Chi_res_fac_dom_main_effects_model_n <- 1 - pchisq(Chi_res_fac_dom_main_effects_model_n, df_res_fac_dom_main_effects_model_n)
prob_Chi_res_fac_dom_main_effects_model_n
```

-   Results here are virtually identical.

```{r}
# Exponentiating the coefficients of the model to reveal the odds ratio conversion of the coefficients
exp(res_fac_dom_main_effects_model_n$coefficients)
```
...
```{r}
# Summarizing the interaction model
summary(res_fac_dom_interaction_model_n)

# Calculating the chi-squared statistic for the model compared to the baseline model with only the intercept
Chi_res_fac_dom_interaction_model_n <- res_fac_dom_interaction_model_n$null.deviance - res_fac_dom_interaction_model_n$deviance
Chi_res_fac_dom_interaction_model_n

# Calculating the degrees of freedom for the chi-square statistic comparing the model to it's baseline
df_res_fac_dom_interaction_model_n <- res_fac_dom_interaction_model_n$df.null - res_fac_dom_interaction_model_n$df.residual
df_res_fac_dom_interaction_model_n

# Conducting a chi-square test for the p-value using the chi-square statistic and the degrees of freedom
prob_Chi_res_fac_dom_interaction_model_n <- 1 - pchisq(Chi_res_fac_dom_interaction_model_n, df_res_fac_dom_interaction_model_n)
prob_Chi_res_fac_dom_interaction_model_n
```
...
```{r}
# Exponentiating the coefficients of the model to reveal the odds ratio conversion of the coefficients
exp(res_fac_dom_interaction_model_n$coefficients)
```
...
```{r}
# Calculating the chi-square statistic to compare the interaction model with the main effects model
Chi_res_fac_dom_interaction_model_n_v_main <- res_fac_dom_main_effects_model_n$deviance - res_fac_dom_interaction_model_n$deviance
Chi_res_fac_dom_interaction_model_n_v_main

# Calculating the degrees of freedom to compare the interaction model with the main effects model
df_res_fac_dom_interaction_model_n_v_main <- res_fac_dom_main_effects_model_n$df.residual - res_fac_dom_interaction_model_n$df.residual
df_res_fac_dom_interaction_model_n_v_main

prob_Chi_res_fac_dom_interaction_model_n_v_main <- 1 - pchisq(Chi_res_fac_dom_interaction_model_n_v_main, df_res_fac_dom_interaction_model_n_v_main)
prob_Chi_res_fac_dom_interaction_model_n_v_main
```
...
```{r}
# Conducting the simple slopes and Johnson-Neyman analysis
sim_slopes(res_fac_dom_interaction_model_n, pred = z_m_res_facial_dominance, modx = z_f_res_facial_dominance, jnplot = TRUE)
```
...
```{r}
# Fitting the model with only the main effects of residual facial dominance for mothers and fathers
res_fac_dom_main_effects_model_n_neutral <- glm(shared_child_sex ~ z_m_res_facial_dominance + z_f_res_facial_dominance + m_num_bio_child, family = binomial(link = logit), data = neutral_face_dyadic_data)

# Fitting the model with the main effects of residual facial dominance for mothers and fathers as well as their interaction
res_fac_dom_interaction_model_n_neutral <- glm(shared_child_sex ~ z_m_res_facial_dominance + z_f_res_facial_dominance + m_num_bio_child + z_m_res_facial_dominance:z_f_res_facial_dominance, family = binomial(link = logit), data = neutral_face_dyadic_data)
```
...
```{r}
# Plotting Cook's distance for the main effects model
plot(res_fac_dom_main_effects_model_n_neutral, which = 4, id.n = 10)

#Plotting Cook's distance for the interaction model
plot(res_fac_dom_interaction_model_n_neutral, which = 4, id.n = 10)
```
...
```{r}
# VIF and tolerance for the main effects model
vif(res_fac_dom_main_effects_model_n_neutral)
1/vif(res_fac_dom_main_effects_model_n_neutral)
```
...
```{r}
# Producing the summary of the main effects model
summary(res_fac_dom_main_effects_model_n_neutral)

# Calculating the chi-squared statistic for the model compared to the baseline model with only the intercept
Chi_res_fac_dom_main_effects_model_n_neutral <- res_fac_dom_main_effects_model_n_neutral$null.deviance - res_fac_dom_main_effects_model_n_neutral$deviance
Chi_res_fac_dom_main_effects_model_n_neutral

# Calculating the degrees of freedom for the chi-square statistic comparing the model to it's baseline
df_res_fac_dom_main_effects_model_n_neutral <- res_fac_dom_main_effects_model_n_neutral$df.null - res_fac_dom_main_effects_model_n_neutral$df.residual
df_res_fac_dom_main_effects_model_n_neutral

# Conducting a chi-square test for the p-value using the chi-square statistic and the degrees of freedom
prob_Chi_res_fac_dom_main_effects_model_n_neutral <- 1 - pchisq(Chi_res_fac_dom_main_effects_model_n_neutral, df_res_fac_dom_main_effects_model_n_neutral)
prob_Chi_res_fac_dom_main_effects_model_n_neutral
```
...
```{r}
# Exponentiating the coefficients of the model to reveal the odds ratio conversion of the coefficients
exp(res_fac_dom_main_effects_model_n_neutral$coefficients)
```
...
```{r}
# Summarizing the interaction model
summary(res_fac_dom_interaction_model_n_neutral)

# Calculating the chi-squared statistic for the model compared to the baseline model with only the intercept
Chi_res_fac_dom_interaction_model_n_neutral <- res_fac_dom_interaction_model_n_neutral$null.deviance - res_fac_dom_interaction_model_n_neutral$deviance
Chi_res_fac_dom_interaction_model_n_neutral

# Calculating the degrees of freedom for the chi-square statistic comparing the model to it's baseline
df_res_fac_dom_interaction_model_n_neutral <- res_fac_dom_interaction_model_n_neutral$df.null - res_fac_dom_interaction_model_n_neutral$df.residual
df_res_fac_dom_interaction_model_n_neutral

# Conducting a chi-square test for the p-value using the chi-square statistic and the degrees of freedom
prob_Chi_res_fac_dom_interaction_model_n_neutral <- 1 - pchisq(Chi_res_fac_dom_interaction_model_n_neutral, df_res_fac_dom_interaction_model_n_neutral)
prob_Chi_res_fac_dom_interaction_model_n_neutral
```
...
```{r}
# Exponentiating the coefficients of the model to reveal the odds ratio conversion of the coefficients
exp(res_fac_dom_interaction_model_n_neutral$coefficients)
```
...
```{r}
# Calculating the chi-square statistic to compare the interaction model with the main effects model
Chi_res_fac_dom_interaction_model_n_v_main_neutral <- res_fac_dom_main_effects_model_n_neutral$deviance - res_fac_dom_interaction_model_n_neutral$deviance
Chi_res_fac_dom_interaction_model_n_v_main_neutral

# Calculating the degrees of freedom to compare the interaction model with the main effects model
df_res_fac_dom_interaction_model_n_v_main_neutral <- res_fac_dom_main_effects_model_n_neutral$df.residual - res_fac_dom_interaction_model_n_neutral$df.residual
df_res_fac_dom_interaction_model_n_v_main_neutral

prob_Chi_res_fac_dom_interaction_model_n_v_main_neutral <- 1 - pchisq(Chi_res_fac_dom_interaction_model_n_v_main_neutral, df_res_fac_dom_interaction_model_n_v_main_neutral)
prob_Chi_res_fac_dom_interaction_model_n_v_main_neutral
```
...
```{r}
# Conducting the simple slopes and Johnson-Neyman analysis
sim_slopes(res_fac_dom_interaction_model_n_neutral, pred = z_m_res_facial_dominance, modx = z_f_res_facial_dominance, jnplot = TRUE)
```
